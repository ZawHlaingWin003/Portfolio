    ...
    <div class="container">
        <nav aria-label="breadcrumb" class="mb-4 breadcrumb-links">
            <ol class="breadcrumb">
                ...
                @if($request->search)
                    <li class="breadcrumb-item active"><a href="">Search Keyword = {{ $request->search }}</a></li>
                @elseif ($request->category)
                    <li class="breadcrumb-item active"><a href="">Category = {{ $request->category }}</a></li>
                @elseif ($request->author)
                    <li class="breadcrumb-item active"><a href="">Author = {{ $request->author }}</a></li>
                @endif
            </ol>
        </nav>
                    ...
                    <div class="blog-item">
                        ...
                        <div class="blog-detail">
                            <div class="blog-title">
                                <h1><a href="{{ route('blogs.show', $blogs[0]) }}">{{ $blogs[0]->title }}</a></h1>
                            </div>
                            <div class="blog-info">
                                <div class="author">
                                ...
                                <div class="category">
                                    <a href="{{ route('blogs.index', ['category' => $blogs[0]->category->name]) }}">
                            ...
                            <div class="blog-btn">
                                <a href="{{ route('blogs.show', $blogs[0]) }}" class="line-btn">Read More &rarr;</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="post-item">
                        <div class="post-img">
                            ...
                            <div class="icons">
                                ...
                                <a href="{{ route('blogs.index', ['author' => $blog->author->name]) }}"> <i class="fas fa-user"></i> by {{ $blog->author->name }}</a>
                            </div>
                        </div>
                        <div class="post-info">
            ...
            <div class="side-bar">
                <div class="categories">
                    ...
                    <ul class="categories-list">
                        <li class="categories-item"><a href="{{ url('/blogs') }}" class="">All Blogs ({{ count($allBlogs) }}) &rarr;</a></li>
                        @foreach ($categories as $category)
                            <li class="categories-item"><a href="{{ route('blogs.index', ['category' => $category->name]) }}" class="">{{ $category->name }} ({{ count($category->blogs) }}) &rarr;</a></li>
                        @endforeach
                    </ul>
                </div>
                <div class="about-me">
